...
public class ShipAIController extends ShipController {
	...
	private CargoObject searchTargetCargo() {
		Ship machine = getMachine();
		...
		if (target == null && !machine.allCargosOccupied()) {
			float maxValue = 0f;
			for (int i = 0; i < list.size(); i++)
				if (list.get(i) instanceof Cargo) {
					Cargo current = (Cargo) list.get(i);
					if (!current.isAlive() || current.getParent() != null || current.getParent() == machine)
						continue;
					float distSq = Maths.distSq(machine.getPos(), current.getPos());
					if (distSq < minDistSq && current.getType().getValue() > maxValue) {
						minDistSq = distSq;
						maxValue = current.getType().getValue();
						target = current;
					}
				}
		}
