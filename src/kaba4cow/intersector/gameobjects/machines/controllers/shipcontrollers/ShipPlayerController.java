package kaba4cow.intersector.gameobjects.machines.controllers.shipcontrollers;
...
public class ShipPlayerController extends ShipController {
	...
	public void update(float dt) {
		...
		if (Input.isKey(Keyboard.KEY_SPACE))
			getMachine().shootManualWeapon();
		if (Input.isKey(Keyboard.KEY_LCONTROL))
			getMachine().shootAutomaticWeapon();
		...
		if (Input.isButton(0))
			getMachine().smoothRotateTo(WindowUtils.toNormalizedX(Mouse.getX()),
	...
	public void selectTarget() {
		List<Target> targetList = Target.getOnScreenObjects(getMachine(), mode.getTargets());
		...
		for (int i = 0; i < targetList.size(); i++) {
			...
			if (current.getScreenCoords().z > minDist || current.getObject().getCollisionSize() < maxSize
					|| Maths.dist(mouseX, mouseY, current.getScreenCoords().x, current.getScreenCoords().y) > 0.1f)
				continue;
