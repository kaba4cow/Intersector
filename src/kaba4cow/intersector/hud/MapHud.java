...
public final class MapHud {
	...
	static {
		...
		HUD_MODEL = new TexturedModel(RawModelContainer.get("MISC/maphud"),
				null);
		SCREEN_MODEL = new TexturedModel(
				RawModelContainer.get("MISC/maphudscreen"), ModelTextureFile
						.get("SCREEN").get());
		HUDTEXT_MODEL = new TexturedModel(
				RawModelContainer.get("MISC/maphudtext"), ModelTextureFile.get(
						"HUDTEXT1").get());
		...
		int i = 0;
		for (int y = 0; y < 3; y++)
			TEXTS[i++] = new GUIText("", font, new Vector2f(0f, -0.55f - 0.15f
					* y), TEXT_SCALE, 1f, true);

		for (int y = 0; y < 3; y++)
			TEXTS[i++] = new GUIText("", font, new Vector2f(0f, -0.05f - 0.15f
					* y), TEXT_SCALE, 1f, true);

		for (int y = 0; y < 3; y++)
			TEXTS[i++] = new GUIText("", font, new Vector2f(0f,
					0.45f - 0.15f * y), TEXT_SCALE, 1f, true);
	}

	public static void renderGalHud(Ship ship, List<SystemObject> systems,
			SystemObject home, SystemObject target, SystemObject cursor) {
		startRendering();
	...
	public static void renderSysHud(Ship ship, List<Planet> planets,
			SystemObject system, PlanetObject target) {
		startRendering();
	...
	private static void renderSysHudText(SystemObject system,
			PlanetObject target) {
		...
		TEXTS[4].setTextString("Sector: [" + system.posX
				/ GalaxyUtils.SECTOR_SIZE + ", " + -system.posY
				/ GalaxyUtils.SECTOR_SIZE + ", " + system.posZ
				/ GalaxyUtils.SECTOR_SIZE + "]");
		TEXTS[5].setTextString("Position: [" + system.posX + ", "
	...
	private static void renderGalHudText(SystemObject home, SystemObject target) {
		...
		if (home != null) {
			...
			TEXTS[7].setTextString("Sector: [" + home.posX
					/ GalaxyUtils.SECTOR_SIZE + ", " + -home.posY
					/ GalaxyUtils.SECTOR_SIZE + ", " + home.posZ
					/ GalaxyUtils.SECTOR_SIZE + "]");
			TEXTS[8].setTextString("Position: [" + home.posX + ", "
					+ -home.posY + ", " + home.posZ + "]");
		}
		...
		if (target == null) {
			...
			TEXTS[1].setTextString("Star: "
					+ target.mainObjects[0].file.getName());
			TEXTS[2].setTextString("System size: " + target.systemSize());

			TEXTS[3].setTextString("Name: " + target.getTargetDescription());
			TEXTS[4].setTextString("Sector: [" + target.posX
					/ GalaxyUtils.SECTOR_SIZE + ", " + -target.posY
					/ GalaxyUtils.SECTOR_SIZE + ", " + target.posZ
					/ GalaxyUtils.SECTOR_SIZE + "]");
			TEXTS[5].setTextString("Position: [" + target.posX + ", "
	...
	private static void renderGalTargets(List<SystemObject> systems,
			SystemObject home, SystemObject target, SystemObject cursor) {
		...
		for (int i = 0; i < targetList.size(); i++) {
			...
			size = 0.5f * Maths.limit(object.getSize() / screenCoords.z, 0.1f,
					1f);
			screenCoords.z = 0f;
			...
			if (object.equals(home))
				RenderUtils.renderHologram(hologramColorInfo, hudTexture,
						target0, 2f, scale, null, matRotated, renderers);
			if (object.equals(target))
				RenderUtils.renderHologram(hologramColorInfo, hudTexture,
						target1, 2f, scale, null, matRotated, renderers);
			if (object == cursor)
				RenderUtils.renderHologram(hologramColorInfo, hudTexture,
						target2, 2f, scale, null, matRotated, renderers);
			if (object.fraction != null)
				RenderUtils.renderHologram(object.fraction.getFractionFile()
						.getMainColor(), hudTexture, target3, 2f, scale, null,
						matStatic, renderers);
		}
	...
	private static void renderSysTargets(List<Planet> planets,
			PlanetObject target) {
		RawModel target2 = RawModelContainer.get("MISC/systarget2");
		ModelTexture hudTexture = ModelTextureFile.get("HUD").get();

		List<Target> targetList = Target.getOnSysMapObjects(planets,
				GameUtils.getMapPov());
		Matrix4f matStatic = null;
		// Matrix4f matRotated = null;
		Target current = null;
		...
		for (int i = 0; i < targetList.size(); i++) {
			...
			if (object.station)
				RenderUtils.renderHologram(object.system.fraction
						.getFractionFile().getMainColor(), hudTexture, target2,
