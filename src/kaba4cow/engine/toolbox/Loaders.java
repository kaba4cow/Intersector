...
public class Loaders {
	...
	public static ModelData getModelData(File file) {
		...
		try {
			...
			return new ModelData(verticesArray, texturesArray, normalsArray,
					indicesArray);
		} catch (IOException e) {
			e.printStackTrace();
			return null;
	...
	public static RawModel loadToVAO(float[] positions, float[] normals,
	...
	private static void storeDataInAttributeList(int vao, int attributeNumber,
	...
	public static int loadTexture(String file, boolean linearSampling) {
		...
		try {
			...
			GL11.glTexParameteri(GL11.GL_TEXTURE_2D,
					GL11.GL_TEXTURE_MAG_FILTER, sampling);
			GL11.glTexParameteri(GL11.GL_TEXTURE_2D,
					GL11.GL_TEXTURE_MIN_FILTER, sampling);
	...
	public static int createTexture(int size, boolean linearSampling) {
		...
		GL11.glTexImage2D(GL13.GL_TEXTURE0, 0, GL11.GL_RGBA, size, size, 0,
	...
	public static int loadCubemapTexture(String... files) {
		...
		for (int i = 0; i < 6; i++) {
			...
			GL11.glTexImage2D(GL13.GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0,
					GL11.GL_RGBA, data.width, data.height, 0, GL11.GL_RGBA,
	...
	public static int createCubemapTexture(int size) {
		...
		for (int i = 0; i < 6; i++) {
			GL11.glTexImage2D(GL13.GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0,
					GL11.GL_RGBA, size, size, 0, GL11.GL_RGBA,
	...
	public static class ModelData {
		...
		public ModelData(float[] vertices, float[] textures, float[] normals,
				int[] indices) {
