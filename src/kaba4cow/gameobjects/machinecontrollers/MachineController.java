package kaba4cow.gameobjects.machinecontrollers;
...
public abstract class MachineController {
	...
	protected void updateTargets(float dt) {
		...
		if (lostTarget) {
			lostTargetTime += dt;
			if (lostTargetTime > 3f || targetFriend != null
					|| targetEnemy != null || targetCargo != null
					|| targetPlanet != null) {
				lostTarget = false;
				lostTargetTime = 0f;
			}
		...
		if (targetFriend != null)
			if (!GameObject.isAlive(targetFriend) || targetFriend.isDestroyed()
					|| Maths.distSq(pos, targetFriend.getPos()) > maxDistSq) {
				targetFriend = null;
				loseTarget();
			}
		...
		if (targetEnemy != null)
			if (!GameObject.isAlive(targetEnemy) || targetEnemy.isDestroyed()
					|| Maths.distSq(pos, targetEnemy.getPos()) > maxDistSq) {
				targetEnemy = null;
				loseTarget();
			}

		if (targetCargo != null)
			if (!GameObject.isAlive(targetCargo)
					|| targetCargo.getParent() != null
