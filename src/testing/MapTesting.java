...
public class MapTesting extends MainProgram {
	...
	public MapTesting() {
		super("Map Testing", 30, 1000, 800, 0);
		EXIT_ON_KEY = false;
	}
	...
	public void init() {
		...
		SystemFile.load(FileUtils.loadFiles(new File("resources/files/systems/"), null, new ArrayList<String>()));
		PlanetFile.load(FileUtils.loadFiles(new File("resources/files/planets/"), null, new ArrayList<String>()));
		FractionFile.load(FileUtils.loadFiles(new File("resources/files/fractions/"), null, new ArrayList<String>()));
		Fraction.init();
		...
		renderer = GameUtils.getMapPov();
		renderers = new RendererContainer(renderer);

		galCamera = renderer.getCamera();

		galCameraManager = new CameraManager().setDistParameters(1f, 10f, 0.002f, 4f).setPointParameters(2f)
				.setPitchParameters(-Maths.QUARTER_PI, Maths.QUARTER_PI, 0.1f, 8f).setYawParameters(0.1f, 8f);
		galCameraManager.setInitParameters(2f, 0f, 0f).reset().resetParameters();
	...
	public void update(float dt) {
		...
		if (Input.isKeyDown(Keyboard.KEY_RETURN))
			target = GalaxyUtils.getRandomSystem();

		if (Input.isKeyDown(Keyboard.KEY_U)) {
			ParticleSystemManager.clear("GALAXY");
			systems.clear();
		...
		if (showSystemMap) {
			...
			for (int i = 0; i < planets.size(); i++) {
				...
				if (planet.getLight() != null)
					renderer.getLights().get(0).getColor().set(planet.getLight().getColor());
			}
	...
	public SystemObject selectGalacticTarget() {
		List<Target> targetList = Target.getOnGalMapObjects(systems, renderer);
			...
			for (int x = -range; x < range; x++)
				for (int z = -range; z < range; z++) {
					if (Maths.distSq(0f, 0f, 0f, x, 2f * y, z) < rangeSq) {
						system = GalaxyUtils.generateSystem(offX + x, offY + y, offZ + z);
						if (system != null && !systems.contains(system)) {
							systems.add(system);
							system.getParticle(freeSystemParticles);
						}
						nebula = GalaxyUtils.generateNebula(offX + x, offY + y, offZ + z);
